...
class Emception {
  ...
  async init() {
    ...
    const tools = {
      ...
      python: [new Python3Process(processConfig), new Python3Process(processConfig), new Python3Process(processConfig)],
    };
    this.tools = tools;
  ...
  run(...args) {
    ...
    return this._run_process_impl([`/emscripten/${args[0]}.py`, ...args.slice(1)], {
  ...
  _run_process_impl(argv, opts = {}) {
    ...
    if (emscripten_script && this.fileSystem.exists(`${emscripten_script}.py`)) {
      argv = ['/usr/bin/python', '-E', `${emscripten_script}.py`, ...argv.slice(1)];
    }
